...
class Ganf:
    ...
    def train_step_one(self, train_set, learning_rate, batch_size, weight_decay, cuda, c=1.0, lamda=0.):
        # tune matrix A
        h_A_old = np.inf
        max_iter = 20
        rho_max = 1e16
        ...
        for _ in range(max_iter):
            while c < rho_max:
                ...
                pbar.set_description('tuning matrix A')
                for i in range(iters):
                    ...
                    pbar.update(1)
                    pbar.set_postfix_str('h:%.5e, loss:%.5e' % (h, total_loss))
                pbar.close()
